    ...
    Student.find( {nombre: req.body.alumno},
        function(err, student) {
            if (student==false) {
                ...
                Subject.find( {nombre:req.body.nombre, alumno: req.body.alumno},
                    function(err, student) {
                        if (student != false) {
                            ...
                            Subject.update(
                                {nombre: req.body.nombre}, {$push: {alumno: req.body.alumno}}, {upsert: true},
                                function (err, subject) {
                                    if (err)
                                        res.send(err);
...
exports.updateSubject = function(req, res){
    Subject.update( {_id : req.params.subject_id},
        {$set:{nombre : req.body.nombre,alumno: [req.body.alumno]}},
        function(err, subject) {
            if (err)
                res.send(err);
            // Obtine y devuelve todas las asignaturas tras crear una de ellas
